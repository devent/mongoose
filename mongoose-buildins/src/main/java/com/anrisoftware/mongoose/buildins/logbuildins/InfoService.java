...
/**
 * Makes the build-in command {@code info} available as a service.
 * 
...
public class InfoService implements CommandService {
	...
	/**
	 * The unique identifier of the build-in command {@code info}.
	 */
	public static final String ID = "info";

	/**
	 * The command information of the build-in command {@code info}.
	 */
	...
	private static final Module[] MODULES = new Module[] { new AbstractModule() {
		...
		protected void configure() {
			install(new FactoryModuleBuilder().implement(Command.class,
					InfoBuildin.class).build(CommandFactory.class));
		}
	} };
