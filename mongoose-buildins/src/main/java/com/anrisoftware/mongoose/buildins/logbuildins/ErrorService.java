...
/**
 * Makes the build-in command {@code error} available as a service.
 * 
...
public class ErrorService implements CommandService {
	...
	/**
	 * The unique identifier of the build-in command {@code error}.
	 */
	public static final String ID = "error";

	/**
	 * The command information of the build-in command {@code error}.
	 */
	...
	private static final Module[] MODULES = new Module[] { new AbstractModule() {
		...
		protected void configure() {
			install(new FactoryModuleBuilder().implement(Command.class,
					ErrorBuildin.class).build(CommandFactory.class));
		}
	} };
