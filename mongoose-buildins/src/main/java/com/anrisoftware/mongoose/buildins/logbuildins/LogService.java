...
/**
 * Makes the build-in command {@code log} available as a service.
 * 
 * @author Erwin Mueller, erwin.mueller@deventm.org
 * @since 1.0
 */
@ProviderFor(CommandService.class)
public class LogService implements CommandService {

	/**
	 * The unique identifier of the build-in command {@code log}.
	 */
	public static final String ID = "log";

	/**
	 * The command logrmation of the build-in command {@code log}.
	 */
	...
	private static final Module[] MODULES = new Module[] { new AbstractModule() {
		...
		protected void configure() {
			install(new FactoryModuleBuilder().implement(Command.class,
					LogBuildin.class).build(CommandFactory.class));
		}
	} };
