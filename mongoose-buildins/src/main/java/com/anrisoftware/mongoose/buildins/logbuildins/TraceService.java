...
/**
 * Makes the build-in command {@code trace} available as a service.
 * 
 * @author Erwin Mueller, erwin.mueller@deventm.org
 * @since 1.0
 */
@ProviderFor(CommandService.class)
public class TraceService implements CommandService {

	/**
	 * The unique identifier of the build-in command {@code trace}.
	 */
	public static final String ID = "trace";

	/**
	 * The command information of the build-in command {@code trace}.
	 */
	...
	private static final Module[] MODULES = new Module[] { new AbstractModule() {
		...
		protected void configure() {
			install(new FactoryModuleBuilder().implement(Command.class,
					TraceBuildin.class).build(CommandFactory.class));
		}
	} };
