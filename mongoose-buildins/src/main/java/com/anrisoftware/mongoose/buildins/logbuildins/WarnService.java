...
/**
 * Makes the build-in command {@code warn} available as a service.
 * 
 * @author Erwin Mueller, erwin.mueller@deventm.org
 * @since 1.0
 */
@ProviderFor(CommandService.class)
public class WarnService implements CommandService {

	/**
	 * The unique identifier of the build-in command {@code warn}.
	 */
	public static final String ID = "warn";

	/**
	 * The command information of the build-in command {@code warn}.
	 */
	...
	private static final Module[] MODULES = new Module[] { new AbstractModule() {
		...
		protected void configure() {
			install(new FactoryModuleBuilder().implement(Command.class,
					WarnBuildin.class).build(CommandFactory.class));
		}
	} };
