...
/**
 * Makes the build-in command {@code debug} available as a service.
 * 
 * @author Erwin Mueller, erwin.mueller@deventm.org
 * @since 1.0
 */
@ProviderFor(CommandService.class)
public class DebugService implements CommandService {

	/**
	 * The unique identifier of the build-in command {@code debug}.
	 */
	public static final String ID = "debug";

	/**
	 * The command information of the build-in command {@code debug}.
	 */
	...
	private static final Module[] MODULES = new Module[] { new AbstractModule() {
		...
		protected void configure() {
			install(new FactoryModuleBuilder().implement(Command.class,
					DebugBuildin.class).build(CommandFactory.class));
		}
	} };
